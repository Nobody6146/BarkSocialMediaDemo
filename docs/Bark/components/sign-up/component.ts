...
interface ComponentFieldExpressionArgs extends HydrateFieldExpressionArgs<SignUpComponentState, SignUpComponent> {
    this:SignUpComponent;
}
function argName(exp: (x:ComponentFieldExpressionArgs) => any):string {
    return name(exp);
}
function propName(exp: (x:SignUpComponentState) => any):string{
    return name(exp);
}
